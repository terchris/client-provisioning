...
#
# Run in devcontainer: pwsh scripts-win/devcontainer-toolbox/tests/run-tests-build.ps1
...
$SCRIPT_ID          = "devcontainer-toolbox-build-tests"
$SCRIPT_NAME        = "Devcontainer Toolbox Build Tests"
$SCRIPT_VER         = "0.2.0"
...
function Show-Help {
    ...
    Write-Host ""
    Write-Host "Run in devcontainer: pwsh scripts-win/devcontainer-toolbox/tests/run-tests-build.ps1"
    Write-Host ""
    Write-Host "Metadata:"
...
# Paths for cleanup
$extractDir = $null
$intunewinPath = Join-Path $parentDir "devcontainer-toolbox-install.intunewin"
...
try {
    ...
    if (-not $skipRemaining) {
        ...
        if (Test-Path $intunewinPath) {
            Test-Pass "devcontainer-toolbox-install.intunewin exists"
        } else {
            Test-Fail "devcontainer-toolbox-install.intunewin exists" "Not found at $intunewinPath"
            $overallPass = $false
            $skipRemaining = $true
        }
    }
    ...
    if (-not $skipRemaining) {
        ...
        if ($fileSize -gt 0) {
            Test-Pass "devcontainer-toolbox-install.intunewin size ($fileSize bytes)"
        } else {
            Test-Fail "devcontainer-toolbox-install.intunewin size" "File is empty (0 bytes)"
            $overallPass = $false
            $skipRemaining = $true
        }
    }
    ...
    if (-not $skipRemaining) {
        ...
        $extractDir = Join-Path ([System.IO.Path]::GetTempPath()) "devcontainer-extract-$([System.Guid]::NewGuid().ToString('N').Substring(0,8))"
    ...
    if (-not $skipRemaining) {
        ...
        $ps1Files = @("install.ps1", "uninstall.ps1", "detect.ps1", "build.ps1", "devcontainer-init.ps1")
    ...
    if (Test-Path $intunewinPath) {
        ...
        log_info "Removed devcontainer-toolbox-install.intunewin"
    }
}
